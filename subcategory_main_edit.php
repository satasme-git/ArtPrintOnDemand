...
if(isset($_GET['id'])){
    ...
    $edite_sql="SELECT * FROM scategory  WHERE id='{$frameid}' LIMIT 1";
    $edite_query=mysqli_query($conn,$edite_sql);
            if($edite_query){
                if(mysqli_num_rows($edite_query)==1){
                    ...
                    //user not found
                    header('Location:subcategory_main_edit.php?err=user_not_found');
                }
            }else{
                //query unsuccessful
               header('Location:subcategory_main_edit.php?err=query_failed');
            }
}
	...
	if(isset($_POST['submit'])){
		...
		$path='uploads/image/subimage/subcategory';
			...
			if(empty($errors)) {
				...
				$pic=$_FILES['pic']['name'];
				if(empty($pic)){
                $addsql="UPDATE scategory SET name='{$name}',code='{$dis}' WHERE id='{$id}' LIMIT 1";
				}else{
					$addsql="UPDATE scategory SET name='{$name}',pic='{$pic}',code='{$dis}' WHERE id='{$id}' LIMIT 1";
				}
				$result_update = mysqli_query($conn,$addsql);
				if($result_update){
					move_uploaded_file($_FILES['pic']['tmp_name'],$path.$pic);
					header('location:subcategory_main_add.php?update=successfuly');
				}else{
                    header('Location:subcategory_main_edit.php?err=query_failed');
                }
				...
				<div class="content">
					...
					<div class="panel panel-flat">
						...
						<div class="panel-body">
							...
							<form class="form-horizontal form-validate-jquery" action="subcategory_main_edit.php" method="POST" enctype="multipart/form-data">
                            ...
                            <?php
								...
								$image.="<p><b>Picture</b></p><img src='uploads/image/subimage/subcategory/{$frame['pic']}' 1- style='width:20%; height:20%;'>&nbsp</div>";
                                echo $image;
